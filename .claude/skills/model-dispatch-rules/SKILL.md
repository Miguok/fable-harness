---
name: model-dispatch-rules
description: 委派子代理時的派工規則——派工包 7 欄、子代理回報模板、升級與降級的門檻，以及主迴圈不該自己做的粗活判準。觸發時機：要用 Agent／Explore／workflow 委派；預估要讀超過 10 個檔；任務形如「找出所有 X」「盤點整個 Y」；子代理交回結果要驗收；要開 worktree 或平行派工；子代理動了範圍外的檔案。不適用於：不涉及委派的單人任務。
---
