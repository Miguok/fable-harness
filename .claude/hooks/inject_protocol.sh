...
#
# 寫下來但沒有人讀，等於沒寫：`.gate_fail` 在 verify_gate 裡存在了兩個月，
# 全 repo 沒有任何東西打開過它。屍檢是為了讓「安靜的失效」變得看得見，所以它
# 必須被送進上下文，不是躺在檔案裡等人想到去看。
#
# ⚠ 這段註解原本還寫著「三支裡只有一支有屍檢，缺陷分佈正好對上（2 對 24）」。
# **那個因果宣稱已經被撤回**（見 CHANGELOG v1.5.0：那個 2 是錯的、引用的 commit
# 根本沒動過該檔、而用行數解釋的更正也量錯了時點）。撤回的宣稱不能留在一個
# **會裝到使用者機器上、每次開場執行**的檔案裡——這個 repo 的主題就是註解與
# 現實不符，而它自己犯了同一件事。
#
