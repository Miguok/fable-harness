...
- **토큰 효율은 설계의 부산물**——비용 절감은 같은 아키텍처에서 자연스럽게 나오는 것이지 덧붙인 기능이 아닙니다. 위의 계층적 배분 덕분에 비싼 모델의 토큰은 이미 추론에만 쓰이고 무거운 작업은 저렴한 모델로 넘어갑니다. 그 위에, 대립 리뷰의 반대역과 Explore식 검색은 **독립 서브에이전트** 안에서 실행되어 각자 깨끗한 컨텍스트를 가진 채 간결한 결론만 돌려주므로 중간의 대량 토큰이 메인 대화에 눈덩이처럼 쌓이지 않고 버려집니다. 그리고 이 서브에이전트들은 **병렬로** fan-out 되어 각자 작은 컨텍스트만 지니므로 하나의 계속 길어지는 스레드에 쌓이지 않습니다. 이 “계층적 배분＋독립 서브에이전트” 형태는 LLM 토큰 비용을 낮추는 방법으로 널리 알려져 있습니다——다만 Fable 자체는 구체적인 수치를 벤치마크한 적이 없으므로, 여기서 말하는 것은 “왜 절약되는가”이지 특정 퍼센트를 약속하는 것이 아닙니다.
...
> **Windows에서만 검증됨.** 설치 절차, 다섯 개의 hook, `fable_doctor.py`는 현재 Windows에서만 엔드투엔드로 실행했습니다. macOS와 Linux도 동작할 것으로 보이지만([INSTALL.md](INSTALL.md)의 인터프리터 탐지에 분기가 있습니다) 실제로 검증하지는 않았습니다. 두 플랫폼에 설치한다면 `python scripts/fable_doctor.py --home ~ --repo <repo>`로 각 hook이 어떤 인터프리터로 해석되는지, 이 clone에서 등록되었는지, 그리고 마커를 남기는 두 개는 마지막으로 발동한 시각을 확인할 수 있습니다.
