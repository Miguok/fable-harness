...
解釋完了」。10.1% 是量在**修完之後**的：那 24 條缺陷的修法把 `goal_gate`
（603 → 1422 行）與 `wiring_gate`（420 → 852 行）灌大了。缺陷是在 `b28ea35`
那份程式上被發現的，而**當時 `verify_gate` 佔 15.0%**，缺陷佔 7.7%——行數解釋
不完，差了一倍。拿修完的體積去解釋修之前的分佈是循環論證。
...
⚠ 這裡原本寫「約 15 個」，那個數字**湊不出來**：逐條列是「兩閘辨識取樣 1 條 ＋
接線閘 13 個 fixture ＋ 發佈閘 3 條 ＋ 本機制 3 條」，看要不要把 13 個 fixture
算成一條，答案是 9 或 21，沒有一種算法得到 15。寫一個沒有清單的概數，就是這一天
一直在犯的那件事的縮小版。
