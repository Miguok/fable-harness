...
**同類掃描：2 處，已一併處理。** 全 repo 掃「目標閘 28.6%／119 綠回合／30 份
transcript」這個宣稱，只有下表那一列（`CHANGELOG.md:226`）是它；
`verify_gate.py:49` 與 `tests/test_verify_gate.py:389` 提到的「近 30 份真實
transcript」是 2026-08-27 關於改碼形態的另一次量測，同名不同義，不動。

| 閘 | 預言機 | 1.4.3 | **1.5** |
|---|---|---|---|
| 接線閘 | 生成指令**真的執行**，看 commit 有沒有成立、守衛有沒有跑 | 漏擋 **72/92**、誤擋 0 | 漏擋 **0/92**、誤擋 0 |
| 目標閘 | 見下方⚠ | 量不到 | 量不到 |
| 驗證閘 | 見下方⚠ | 量不到 | 量不到 |

接線閘那一列可以重算，做法就在 `tests/test_wiring_oracle.py`（`--sweep` 跑完整
交叉展開）。⚠ 語料大小**隨機器變動**：包裝器維度用 `shutil.which` 偵測，沒有
`winpty` 的機器（＝所有 Linux）拿到的條數較少，而「指令在本機跑不起來」的那些
（本機 24 條，多為 `winpty` 需要 tty）不列入判斷。

⚠ **目標閘那一列被我撤回了。** 它原本寫「30 份真實 transcript、119 個綠回合，
1.4.3 誤擋 34（28.6%）、1.5 誤擋 2（1.7%）」。三個問題，都是發佈前抗辯查出來的：

1. **repo 裡沒有任何東西能重算它**——沒有腳本、沒有測試、沒有可貼上跑的指令。
   而這一版自己立的規矩是「數字旁邊要有產生它的指令」，那條規矩當時只套用在
   行數表上。
2. **它的「獨立預言機」論證與同一份文件矛盾**：驗證閘被判「量不到」的理由是
   「真相要靠 `TEST_CMD_RE` 判，會循環論證」，而目標閘判斷一個回合有沒有跑測試
   用的**就是同一個** `TEST_CMD_RE`。同一個辨識器不能一邊算循環、一邊算獨立。
3. **共用分母 119 兩個版本都產不出來**：各用自身的回合判定去切同一批 transcript，
   1.4.3 得 37 個綠回合、1.5 得 52 個——因為 1.4.3 的回合判定是反的，而那正是
   本版修掉的缺陷。拿一個它不實作的回合模型去替它打分，數字不可比。
   獨立重跑（不同抽樣）得到 1.4.3 3.8%、1.5 0%——方向一致、量級不同，
   **而「量級不同」本身就說明那個數字取決於抽樣，不該當成發佈說明的招牌**。

所以現在只留一個可重算的數字。⚠ 它量的也只有**誤擋**方向；「該擋而沒擋」只有
接線閘量得到，因為只有它的真相（守衛到底有沒有執行）可以向 git 問。
...
- **`_quiet_lock` 的 POSIX 分支從未在任何平台被執行過。** 屍檢的檔案鎖有兩個
  ...
  那一段是**讀碼寫出來、沒有跑過**的程式碼，而這是要發給別人裝的套件。
  ⚠ 同一輪抗辯建議**整段刪掉**，理由查證屬實：拿掉鎖之後全套只有 Q10 會紅
  （實測 1 failed / 446 passed），Q13 仍綠——救它的是「讀不到就直接寫」的回退，
  不是鎖；而殘行進上下文那一半已由注入器的白名單獨立擋住。
  **沒有照做，理由是時機**：那是 165 行產品程式碼、三份副本，在發佈當天動刀，
  而今天的缺陷密度已經證明「發佈日的減法」比「留著一段不參與判定的程式碼」
  更容易生出回歸。列在這裡而不是悄悄留著，下一版第一件事就是處理它。
