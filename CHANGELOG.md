...
修完 26 條之後回頭看，其中**約一半**是同一個決定寫在二十幾個地方：三道閘在
「判不出來」時會安靜，而安靜與「一切正常」在外部完全分不出來。

⚠ 這句話第一版寫成「幾乎全是」，逐條分類之後不成立，改掉了——過度宣稱一個
根因，比沒找到根因更糟，因為它會讓另外一半停止被追究。另一半的類別各自是：
守衛斷言在自己貼的字串上（假綠）、同一段邏輯有多份副本、注入內容沒有框架、
機密遮蔽漏形態、註解與程式不一致、以及兩條**誤擋**（那是相反方向的病）。
安靜這一類之所以值得單獨處理，不是因為它最多，是因為**它是唯一一個「不修就
會一直被重新發現」的類別**：另外那些一修就結案，而安靜的分支可以無限新增。

證據本來就在 repo 裡，只是沒有人這樣看過——

| | fail-open 屍檢 | 本輪抓到的缺陷 |
|---|---|---|
| `verify_gate` | 5 處 | **2** |
| `goal_gate` | 0 處 | 24（與下一列合計） |
| `wiring_gate` | 0 處 | |

三支合計 25 個 `except`，會留下痕跡的**只有 1 個**。所以「再跑一輪抗辯」不會收斂：
每一輪找到的都只是安靜的一個實例，而第 27 條仍然可以被加進來而沒有人發現。

本版改的是**生成器**，不是第 27 個實例：

- `note_quiet()`：一次 fail-open 寫一行到 `.gate_fail`（UTC 時間戳、呼叫點標籤、
  例外類別；上限 500 行且保留**最早**那幾行——第一次靜默死亡最有價值；自己絕不拋
  例外，遙測故障不得把「閘安靜失效」升級成「閘弄壞 session」）。
- 16 個 `except` 留痕；另外 10 個在 `except` 那一行標 `# quiet-ok: <理由>`，因為
  正常控制流不是失效，記它只會淹掉訊號。
- `tests/test_no_silent_gate.py` 走 AST：任何 `except` 既沒留痕、沒 `raise`、也沒
  標記就紅。**新的安靜分支加不進來**，而寫下理由是一個必須經過思考的動作。
- `.gate_fail` 現在**有人讀**。它在 `verify_gate` 裡躺了兩個月，全 repo 沒有任何
  東西打開過它——寫下來卻沒有人看，和沒寫是同一種病。SessionStart 會把筆數與
  最後五行送進上下文。

這條規則當場抓到兩件事，那正是它的用途：`verify_gate` 少了一個 `import os`，
新加的留痕靜默寫不出東西（而它自己的 `except` 把那個錯吞掉了）；第一版連
「這個 repo 沒有宣告驗證指令」也記，一次全套測試就寫了 **120 行**——吵到沒人看的
屍檢和沒有屍檢一樣沒用，而且會把上限灌爆讓真正的失效寫不進去（現在 7 行，
全部來自刻意注入失敗的測試）。

它的突變驗收方式也和別的守衛不同：不是還原某個修法，是**新增一個從來不存在的
安靜分支**，看它會不會叫。

⚠ 它只掃**例外處理**。「不是 except 的安靜早退」（`if not root: return 0` 這類）
還是只有個別情境測試在守，沒有通則守衛——要有的話，需要一種能分辨「這個 return
是放棄判定」與「這個 return 是正常結論」的標記方式，目前沒有，硬掃會把每個正常
`return` 都判成缺陷。這一條寫在 `tests/test_no_silent_gate.py` 的待驗收欄裡。
