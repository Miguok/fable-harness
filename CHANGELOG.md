...
| 「有屍檢的那支本輪只出 2 條缺陷」 | 我引用的那個 commit **根本沒動過** `verify_gate` |
| 2 : 24 的分佈證明屍檢有效 | `verify_gate` 佔三支程式碼的 **10.1%**，缺陷佔 7.7%——用行數就解釋完了，屍檢的有無沒有解釋力 |

逐條走過 `b28ea35..HEAD` 的缺陷，**只有一條**（`.claude/fable-verifier` 的 cp950
位元組）的傷害路徑真的經過 `except`。其餘是正則寫錯、範圍取錯、淘汰策略錯、
遮蔽漏形態、註解與程式不符、假綠守衛，以及 `release.py` 的一批——那些
`note_quiet` 一個都碰不到。

**那什麼才是這一天真正重複出現的？** 是**我為自己的修法配的守衛驗不到東西**。
本輪清點到的實例：兩支 hook 的測試指令辨識比對用取樣（而兩份當時已經歪了）、
接線閘 13 個 fixture 全部把宣告檔寫在註解裡、三條發佈閘的測試斷言在程式沒走到
的那一格、以及這個機制自己的 Q4／Q5／Q6／Q1——約 15 個。
...
記在那個檔案的 docstring 裡：Q4 走不到它宣稱驗的那段程式；Q5 在「遙測整個死掉」
時照樣綠；Q6 只比對我貼的字串，把注入區塊**註解掉**（不是刪掉）它不會叫；
Q1 的判定漏掉 `contextlib.suppress`、把條件式 `raise` 算成往上丟、把巢狀呼叫
算成外層有留痕，而且檔案清單寫死。
