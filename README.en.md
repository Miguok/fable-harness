...
![Version: 1.3.1](https://img.shields.io/badge/version-1.3.1-blue.svg) &nbsp; ![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)
...
> **Tested on Windows.** The install flow, the five hooks and `fable_doctor.py` are exercised on Windows only; macOS and Linux are expected to work (the interpreter detection in [INSTALL.md](INSTALL.md) branches for them) but have not been run end to end. If you install on either, `python scripts/fable_doctor.py --home ~ --repo <repo>` will tell you which interpreter each hook resolved to, whether it is registered from this clone, and — for the two hooks that leave a marker — when they last fired.
